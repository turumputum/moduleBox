...
void delayMS(int ms) {
	...
	//ESP_LOGD(TAG, "ms=%ld _ms=%ld portTICK_PERIOD_MS=%ld xTicksToDelay=%ld", ms, _ms, portTICK_PERIOD_MS, xTicksToDelay);
...
void lcdInit(TFT_t *dev, int width, int height, int offsetx, int offsety) {
	...
	spi_master_write_data_byte(dev, 0b11000000);

	uint16_t x_start = 0 + dev->_offsetx, x_end = 240 + dev->_offsetx;
	uint16_t y_start = 0 + dev->_offsety, y_end = 240 + dev->_offsetx;


	spi_master_write_command(dev, 0x2A);	//Column Address Set
	spi_master_write_data_byte(dev, x_start >> 8);
	spi_master_write_data_byte(dev, x_start & 0xFF);
	spi_master_write_data_byte(dev, x_end >> 8);
	spi_master_write_data_byte(dev, x_end & 0xFF);

	spi_master_write_command(dev, 0x2B);	//Row Address Set
	spi_master_write_data_byte(dev, y_start >> 8);
	spi_master_write_data_byte(dev, y_start & 0xFF);
	spi_master_write_data_byte(dev, y_end >> 8);
	spi_master_write_data_byte(dev, y_end & 0xFF);
...
uint8_t BMP_cashPic(char *file, int width, int height) {
	...
	assert(ret == 1);
	ESP_LOGD(TAG, "result->header.filesz=%ld", result->header.filesz);
	...
	if ((result->dib.depth == 8) && (result->dib.compress_type == 0)) {
		...
		ESP_LOGD(TAG, "Read file complite at:%ld ms ", (xTaskGetTickCount()-startTick)*portTICK_PERIOD_MS);
...
void JPEGTest(TFT_t *dev, char *file, int width, int height) {
	...
	if (err == ESP_OK) {
		...
		ESP_LOGD(TAG, "Jpeg heap usage:%ld free heap:%d", heap_temp - xPortGetFreeHeapSize(), xPortGetFreeHeapSize());
		free(colors);
		release_image(&pixels, width, height);
		ESP_LOGD(TAG, "Finish");
	}else{
		ESP_LOGW(TAG, "Image not found");
		lcdFillScreen(dev, BLACK);
	}

	endTick = xTaskGetTickCount();
	diffTick = endTick - startTick;
	ESP_LOGI(TAG, "elapsed time[ms]:%ld heap usage:%ld free heap:%d", diffTick*portTICK_PERIOD_MS, heapBefore - xPortGetFreeHeapSize(), xPortGetFreeHeapSize());
}
