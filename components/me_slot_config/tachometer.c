...
void tachometer_task(void *arg)
{
	...
	if (strstr(me_config.slot_options[num_of_slot], "threshold")!=NULL){
		...
		if (threshold <= 0)
		{
			ESP_LOGD(TAG, "threshold wrong format, set default. Slot:%d", num_of_slot);
	...
	if (strstr(me_config.slot_options[num_of_slot], "custom_topic")!=NULL){
		...
		//get_option_string_val(num_of_slot,"custom_topic", &custom_topic);
	...
	if(flag_custom_topic==0){
		...
		sprintf(str, "%s/tachometer_%d",me_config.device_name, num_of_slot);
	...
	while (1)
	{
		...
		if(flag_report){
			...
			if(flag_custom_topic){
				...
				str = (char*)malloc(str_len * sizeof(char));
				sprintf(str,"%s/tachometer_%d:%d", me_config.device_name, num_of_slot, resault);
			}
...
void start_tachometer_task(int num_of_slot){
	...
	ESP_LOGD(TAG, "tachometer_task init ok: %d Heap usage: %lu free heap:%u", num_of_slot, heapBefore - xPortGetFreeHeapSize(), xPortGetFreeHeapSize());
}