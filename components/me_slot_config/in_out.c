...
void out_3ch_task(void *arg) {
	...
	uint32_t heapBefore = xPortGetFreeHeapSize();
	//---init hardware---
	uint8_t pin_0 = SLOTS_PIN_MAP[slot_num][0];
	esp_rom_gpio_pad_select_gpio(pin_0);
	gpio_set_direction(pin_0, GPIO_MODE_OUTPUT);

	uint8_t pin_1 = SLOTS_PIN_MAP[slot_num][1];
	esp_rom_gpio_pad_select_gpio(pin_1);
	gpio_set_direction(pin_1, GPIO_MODE_OUTPUT);

	uint8_t pin_2 = SLOTS_PIN_MAP[slot_num][2];
	esp_rom_gpio_pad_select_gpio(pin_2);
	...
	gpio_set_level(pin_1, (uint32_t )def_state);
	...
	while(1){
		...
		if (xQueueReceive(me_state.command_queue[slot_num], &msg, portMAX_DELAY) == pdPASS){
			...
			int cmd_pin_num=pin_1;
			if(strstr(msg.str, "ch_1")!=NULL){
				cmd_pin_num = pin_0;
			}else if(strstr(msg.str, "ch_2")!=NULL){
				cmd_pin_num = pin_1;
			}else if(strstr(msg.str, "ch_3")!=NULL){
				cmd_pin_num = pin_2;
			}
			//ESP_LOGD(TAG, "incoming command val:%d", val);
			out_level_cmd_t port_cmd={
				.gpio_num=cmd_pin_num,
				.level=out_inverse ? !val : val,
				.slot_num = slot_num
			};
		...
		if(state_mass[i]!=prev_state_mass[i]){
			...
			if(logic==INDEPENDENT_MODE){
				...
				if(summ==0){
					...
					if(prev_state_mass[0]==0&&prev_state_mass[1]==0&&prev_state_mass[2]==0){
						sprintf(t_str, "%d", 1);
					}else{
						goto skip;
					}
				...
				if(summ==numOfCh){
					...
					if(prev_state_mass[0]==1 && prev_state_mass[1]==1 && prev_state_mass[2]==1){
						sprintf(t_str, "%d", 0);
					}else{
						goto skip;
					}
