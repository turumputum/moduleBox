...
#define BUF_SIZE 256
    ...
    if (strstr(me_config.slot_options[num_of_slot], "dead_band") != NULL) {
        ...
        if (deadBand <= 0) {
            ...
            ESP_LOGD(TAG, "TOF set dead_band:%d for slot:%d", deadBand, num_of_slot);
        }
    }
    ...
    if (strstr(me_config.slot_options[num_of_slot], "threshold") != NULL) {
        threshold = get_option_int_val(num_of_slot, "threshold");
        if (threshold <= 0)
        {
            ESP_LOGE(TAG, "threshold wrong format, set default. Slot:%d", num_of_slot);
    ...
    if (flag_custom_topic == 0) {
        ...
        sprintf(str, "%s/distance_%d", me_config.device_name, num_of_slot);
    ...
    while (1) {
        ...
        if (f_report==1) {
            //ESP_LOGD(TAG,"TOF:%d distance is:%d",num_of_slot,dist);
            f_report = 0;
            ...
            if (flag_custom_topic) {
                ...
                if (flag_float_output) {
                    sprintf(str, "%s/distance_%d:%f", me_config.device_name, num_of_slot, f_res);
                }else {
                    sprintf(str, "%s/distance_%d:%d", me_config.device_name, num_of_slot, resault);
                }
            }
            report(str);
...
void start_benewakeTOF_task(int num_of_slot) {
    ...
    xTaskCreate(benewakeTOF_task, "benewakeTOF_task", 1024 * 4, &num_of_slot, 5, NULL);
    ESP_LOGD(TAG, "Lidar init ok: %d Heap usage: %lu free heap:%u", num_of_slot, heapBefore - xPortGetFreeHeapSize(), xPortGetFreeHeapSize());
}