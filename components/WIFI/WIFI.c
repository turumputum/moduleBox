...
uint8_t wifiInit() {
	...
	if(me_config.WIFI_enable==1){
		...
		if (me_config.WIFI_DHCP == 0) {
			...
			esp_netif_str_to_ip4((const char*) me_config.WIFI_ipAdress, &info_t.ip);
			esp_netif_str_to_ip4((const char*) me_config.WIFI_gateWay, &info_t.gw);
			esp_netif_str_to_ip4((const char*) me_config.WIFI_netMask, &info_t.netmask);
