...
void configure_mp3Player(PAUDIOCONFIG c, int slot_num)
{
	...
	*/
	...
	if (strstr(me_config.slot_options[slot_num], "topic") != NULL) {
		...
		char t_action[strlen(custom_topic) + 8];
		sprintf(t_action, "%s/action", custom_topic);
		char t_event[strlen(custom_topic) + 7];
		...
		char t_str[strlen(me_config.deviceName)+strlen("/player_0")+10];
		sprintf(t_str, "%s/player_%d/action",me_config.deviceName, slot_num);
		me_state.action_topic_list[slot_num]=strdup(t_str);
		sprintf(t_str, "%s/player_%d/event",me_config.deviceName, slot_num);
