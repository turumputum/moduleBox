// ***************************************************************************
// TITLE
//      HLK-LD2410 Radar Distance Sensor Module
//
// PROJECT
//     moduleBox
// ***************************************************************************
...
#include <generated_files/gen_distanceSens_hlk2410.h>
...
void configure_hlk2410(distanceSens_t *distanceSens, uint8_t slot_num)
{
    ...
    if (strstr(me_config.slot_options[slot_num], "deadBand") != NULL) {
        ...
        if (distanceSens->deadBand <= 0) {
            ESP_LOGD(TAG, "Radar dead_band wrong format, set default slot:%d", distanceSens->deadBand);
            distanceSens->deadBand = 1;
        } else {
            ESP_LOGD(TAG, "Radar set dead_band:%d for slot:%d", distanceSens->deadBand, slot_num);
        }
    }
    ...
    /* Максимальное значение диапазона измерений в см, По умолчанию 800 */
    if (strstr(me_config.slot_options[slot_num], "maxVal") != NULL) {
        distanceSens->maxVal = get_option_int_val(slot_num, "maxVal", "", 800, 1, 4096);
        ESP_LOGD(TAG, "Set max_val:%d. Slot:%d", distanceSens->maxVal, slot_num);
    }

    /* Минимальное значение диапазона измерений в см, По умолчанию 0 */
    ...
    /* Порог для бинарного выхода - при 0 работает как аналоговый датчик, По умолчанию 0 */
    ...
    /* Коэффициент фильтра сглаживания от 0 до 1 - при 1 фильтр отключен, По умолчанию 1 */
...
void hlk2410_task(void* arg) {
    ...
    configure_hlk2410(&distanceSens, slot_num);
...
const char * get_manifest_distanceSens_hlk2410()
{
	return manifesto;
}
