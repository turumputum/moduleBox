...
void configure_adc1(PADC1_CHANNEL	ch, int slot_num)
{
    ...
    if (strstr(me_config.slot_options[slot_num], "filterK")!=NULL){
        ...
        ch->k = get_option_float_val(slot_num, "filterK", 0, 0.0, 5.0);
    ...
    if (strstr(me_config.slot_options[slot_num], "periodic")!=NULL){
        ...
        ch->periodic = get_option_int_val(slot_num, "periodic", "ms", 10, 1, 4096);
...
/* 
    Модуль реализует поддержку второго ADC
*/
void configure_adc2(PADC1_CHANNEL	ch, int slot_num)
{
	ch->slot_num 			= slot_num;
    ch->MIN_VAL 			= 0;
    ch->MAX_VAL 			= 4095;
