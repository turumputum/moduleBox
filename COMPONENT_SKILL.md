...
## 7.5 ЦЕЛЫЕ ЧИСЛА ПО УМОЛЧАНИЮ, FLOAT — В КРАЙНЕМ СЛУЧАЕ

**Значения передаются целыми числами. `float` — исключение, а не выбор по вкусу.**

Дробную величину масштабируют в целое и указывают единицу в манифесте:
температура в милиградусах (`24500` = 24,5 °C), напряжение в милливольтах,
проценты в сотых долях. Величина остаётся точной, а вся арифметика — целой.

Почему так:

- `stdcommand` сверяет типы строго: параметр, объявленный `PARAMT_int`,
  **отвергает** значение с точкой, и команда молча не выполняется. Параметр
  `PARAMT_float` принимает и то и другое, поэтому смешение типов в цепочке
  модулей всплывает не сразу, а на чужом конфиге;
- арифметика кросслинка (`@*3.8+1000`) отдаёт результат целым — иначе
  принимающая команда с `PARAMT_int` его отбросит. Дробный выход модуля
  в такой цепочке работать не будет;
- `float` — это 24 бита мантиссы. Выше ~16,7 млн значения округляются, и
  счётчики, позиции шаговиков и большие диапазоны начинают врать;
- целое печатается коротко и однозначно. `%f` даёт `24.500000` — шесть знаков
  мусора в каждом сообщении MQTT.

Где `float` уместен:

- на границе с чужим драйвером, который отдаёт `float` (например
  `ds18b20_get_temperature`). Преобразуйте сразу: `lroundf(t * 1000.0f)` —
  и дальше по модулю только целые;
- промежуточные вычисления, где целочисленное переполнение или потеря
  точности хуже (коэффициенты, гамма-коррекция, тригонометрия);
- отдельная опция модуля, явно включающая дробный выход, — как `floatOutput`
  в `scaler`. По умолчанию всё равно целое.

Округление — `lroundf()`, к ближайшему. Приведение `(int)x` усекает к нулю и
систематически занижает результат на единицу.
...
### 11.0 ОБЯЗАТЕЛЬНО: nSLEEP драйвера выходов

Третья ножка слота, `SLOTS_PIN_MAP[slot_num][2]`, заведена на вход **nSLEEP
микросхемы драйвера выходов**. Пока она не поднята в единицу, драйвер держит
выход в высоком импедансе: модуль работает, команды принимает, пины дёргает —
а наружу не выходит ничего. Симптом при этом ровно нулевой, в логе чисто.

**Правило.** Любой модуль, который выдаёт сигнал наружу и сам ножку с индексом 2
не использует, обязан вызвать:
...
Место вызова — в task-функции, рядом с настройкой своих GPIO, перед
`waitForWorkPermit()`.

Функция сама проверяет номер слота и пропускает виртуальные слоты 6–9, где в
карте пинов нули: ноль означает отсутствие ножки, а не GPIO0.

**Кто НЕ зовёт.** Модули, у которых индекс 2 занят собственным сигналом, —
`pwmLeds` (канал B), `out_3ch` и `in_3ch` (третий канал), `distanceSens`,
`wavPlayer`, `encoderAS5600`, `lidars`, `analog` (делитель). Там ножкой
распоряжается сама схема слота, и поднимать её нельзя.

**Кто зовёт сейчас:** `PWMgenerator`, `PPMservo`, `PPM`, `stepper`, `in_out`,
`out_2ch`, `relay`, `button_led`, `button_smartLed`, `button_ledRing`,
`button_ledBar`, `button_runFire`, `button_swiperLed`.
...
**Краткость обязательна.** Описание показывается в конфигураторе как подсказка на
маленькой плашке — одна короткая строка, без многострочных списков. Для опций
кратко указывай значение по умолчанию в формате `По умолчанию N` (для флагов
`По умолчанию активен`, для пресенс-опций `По умолчанию unicast`):
```c
/* Громкость 0-100, По умолчанию 100
*/
ctx->volume = get_option_int_val(slot_num, "volume", "percent", 100, 0, 100);
```

**Потолок `refreshRate` — 100 Гц.** Частота обработки данных модуля не должна
превышать 100 раз в секунду (проектное решение по нагрузке, не предел тика — тик
FreeRTOS 1000 Гц). Для опции частоты обновления ставь `max=100` в
`get_option_int_val`.
...
#### ⚠️ Ограничения на текст комментариев

Комментарии для генерации манифеста парсятся инструментом `manifesto`. Текст описания вставляется напрямую в JSON-строку (`"description": "%s"`) без экранирования — поэтому следующие символы **запрещены** внутри текста таких комментариев:
...
**Безопасны** и могут использоваться свободно: `/`, `|`, `-`, `_`, `(`, `)`.

❌ Неправильно:
```c
/* Скорость в мм/с. Диапазон от 0 до 100.
*/
/* Задаёт режим "авто"
*/
/* Путь C:\folder\file
