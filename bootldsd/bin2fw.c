...
uint32_t crc32buf(uint32_t oldcrc32, char *buf, size_t len)
{
    for ( ; len; --len, ++buf)
    {
...
int main(int argc, char *argv[])
{
    ...
    FILE *              outf;
    int                 rd;
    UPDATEHEAD          hdr         = { UPDATE_MAGIC,  0, 0xFFFFFFFF, 0 };
    uint8_t             buff        [ 4 * 1024 ];
    ...
    if (argc > 1)
    {
        if ((inf = fopen(argv[1], "rb")) != NULL)
        {
            if ((outf = fopen(UPDATE_FILENAME, "w+b")) != NULL)
            {
                ...
                while ((rd = fread(buff, 1, sizeof(buff), inf)) > 0)
                {
                    hdr.checksum = crc32buf(hdr.checksum, buff, rd);

                    fwrite(buff, 1, rd, outf);
